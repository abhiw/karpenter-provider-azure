...
func (c *CloudProvider) vmInstanceToNodeClaim(ctx context.Context, vm *armcompute.VirtualMachine, instanceType *cloudprovider.InstanceType) (*karpv1.NodeClaim, error) {
	// Defense-in-depth: callers MUST hand us a non-nil vm. If createFleetInstance
	// were to read FleetMemberPromise.VM before Wait() populated it, that would pass
	// nil here and crash on *vm.Name below. Surface a clear error instead of an
	// opaque nil-pointer panic so any future regression is obvious in the controller logs.
	if vm == nil {
